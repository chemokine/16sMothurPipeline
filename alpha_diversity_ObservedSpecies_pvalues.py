...
"""
Run statistical testing on shannon values between groups:
Observed species values were calculated on filtered 0.03Cluster OTU table for each sample
...
if len(sys.argv)!=5:
    raise Exception ("Useage of alpha_diversity_ObservedSpecies_pvalues.py <input observed species value table> <comparison groups list> <R script full path> <ouput pvalue table file>")
...
for line in infh:
    ...
    if count>1:
        ...
        outfh.write("x<-data$ObservedSpecies[data$CohortGroups=='"+x_name+"']\n")
        outfh.write("y<-data$ObservedSpecies[data$CohortGroups=='"+y_name+"']\n")

...
(2)Generate one folder, including read counts only
...
def get_otu(file):
    ...
    for line in in_fh:
        ...
        for i,v in enumerate(list):
            ...
            #In OTU_taxaInfo from mothur, it has 23.0 value
...
def print_ab_abundance(out_dir,tag):
    ...
    for level,value in dict_taxa.items():
        ...
        dict={}
        dict.clear()
        for taxa in sorted(dict_otu.keys()):
...
def bin_taxa(name):
    #(1)For taxonomic table with read counts
    out_dir=name+'_ab/'
    if os.path.exists(out_dir):
        shutil.rmtree(out_dir)
    os.mkdir(out_dir)
    print_ab_abundance(out_dir,'ab')
...
"""
if len( sys.argv ) != 4:
        raise Exception( "Usage: taxa_summary_table_ab.py <nonefilter file full path or filter OTUs directory full path> <Filter/nonFilter> <work directory>")
